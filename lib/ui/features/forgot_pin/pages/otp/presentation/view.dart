...
class VerifyPinOtpView extends StatelessWidget {
  final VerifySignupOtpArgs args;
  const VerifyPinOtpView({super.key, required this.args});

  @override
  Widget build(BuildContext context) {
    return BaseView<VerifyPinOtpViewModel>(
      onModelReady: (model) => model.init(context, args.phone),
      model: VerifyPinOtpViewModel(),
  ...
  List<Widget> titleAndSubtitle() {
    return [
      ...
      SizedBox(
        width: 350,
        child: Text(
          "Please enter the OTP code sent to the number ending in ${args.phone.substring(args.phone.length - 4)}",
